(function($, window, document) {
	...
	$.PullToRefresh = $.Class.extend({
		init: function(element, options) {
			...
			this.options = $.extend(true, {
				...
				up: {
					...
					show: true,
					contentinit: '上拉显示更多',//
					contentdown: '上拉显示更多',//上拉显示更多
					contentrefresh: '<span class="jhw-loading">加载中...</span>',
			...
			if ($.isFunction(self.options.up.callback)) {
				self.pullUpTips = (function() {
					...
					if (!element) {
						...
						element.innerHTML = '<div class="mui-pull-bottom-wrapper"><span class="mui-pull-loading  666">' + self.options.up.contentinit + '</span></div>';
		...
		pullUpLoading: function(e) {
			...
			this.pullUpTips.classList.add('jhw-active');
		...
		endPullUpToRefresh: function(finished) {
			if (finished) {
				...
				this.pullUpTips.classList.remove('jhw-active');
			}
			this.loading = false;
			if (this.isInScroll) {
				$(this.element.parentNode).scroll().refresh();
