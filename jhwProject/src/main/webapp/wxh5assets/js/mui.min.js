...
function() {
	...
	function(a, b) {
		...
		a.toast = function(b, d) {
			...
			if(d = a.extend({
				...
				return f.classList.add("jhw-"+d.type+"-container"), f.innerHTML = '<div class="jhw-'+d.type+'-message">' + b + "</div>", f.addEventListener("webkitTransitionEnd", function() {
	...
	function(a, b) {
		...
		a.hint = function(b, d) {
			...
			if(d = a.extend({
					duration: "short",
					_parentNode:document.body
				}, d || {}), !a.os.plus || "div" === d.type) {
				"number" == typeof d.duration ? duration = d.duration > 0 ? d.duration : e["short"] : duration = e[d.duration], duration || (duration = e["short"]);
				var f = document.createElement("div");
				return f.classList.add("jhw-hint-container"), f.innerHTML = '<div class="jhw-hint-message">' + b + "</div>", f.addEventListener("webkitTransitionEnd", function() {
					f.classList.contains(c) || (f.parentNode.removeChild(f), f = null)
				}), f.addEventListener("click", function() {
					f.parentNode.removeChild(f), f = null
				}), d._parentNode.appendChild(f), f.offsetHeight, f.classList.add(c), setTimeout(function() {
					f && f.classList.remove(c)
				}, duration), {
					isVisible: function() {
						return !!f
	...
	function(a, b) {
		...
		a.finish = function(b, d) {
			...
			if(d = a.extend({
				...
				"number" == typeof d.duration ? duration = d.duration > 0 ? d.duration : e["short"] : duration = e[d.duration], duration || (duration = e["short"]);
				var f = document.createElement("div");
				return f.classList.add("jhw-finish-container"), f.innerHTML = '<div class="jhw-finish-message">' + b + "</div>", f.addEventListener("webkitTransitionEnd", function() {
					f.classList.contains(c) || (f.parentNode.removeChild(f), f = null)
				}), f.addEventListener("click", function() {
					f.parentNode.removeChild(f), f = null
				}), document.body.appendChild(f), f.offsetHeight, f.classList.add(c), setTimeout(function() {
	...
	function(a, b, c) {
		var d = "mui-popup",
			...
			r = function(a) {
				return '<div class="' + k + '"><textarea placeholder = "' + (a || "") + '"></textarea></div>'
			},
			s = function(a, b, c) {
		...
		t.prototype.init = function() {
			...
			if(this.sliderActionClass) {
				...
				this.searchActionClass && (b.addEventListener("focus", function() {
					...
					b.parentNode.parentNode.parentNode.classList.add("jhw-active");
				}), b.addEventListener("blur", function() {
					b.value.trim() || b.parentNode.classList.remove("mui-active");
					b.value.trim() || b.parentNode.parentNode.parentNode.classList.remove("jhw-active");
				}))
			}
